  ...
  void CommonRadioInterface::Run()
  {
    ...
    while (1)
    {
      // Package pending network frames into radio frames
      {
        ...
        if (!pending_network_frame_buffer_.empty())
        {
          ...
          for (int i = 0; i < frame.size(); i += 30)
          {
            ...
            //LOGS("Packing Radio Frame - ID: %d, Bytes: %d", radio_frame.packet.id, radio_frame.packet.size);
          }
      ...
      if (!pending_radio_frame_buffer_.empty())
      {
        ...
        {
          // LOGS("Last time sent: %llu, Current time: %llu", radio_frame.last_time_sent, TimeNowUs());
          ...
          //LOGS("Sending radio frame - ID: %d, Bytes: %d", radio_frame.packet.id, radio_frame.packet.size);
          RadioSend(request);
        }
      }
      ...
      case RequestResult::Success:
      {
        ...
        //LOGS("Received packet - Type: %d, ID: %d, Size: %d", response_packet->type, response_packet->id, response_packet->size);
        ...
        case packet_type::DATA:
        {
          ...
          {
            LOGS("Received final packet - ID: %d, Bytes: %d", response_packet->id, response_packet->size);
